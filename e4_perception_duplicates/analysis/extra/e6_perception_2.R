...
#save mat of max(self1, self2) for the both self condition
d.mat_b_final <- array(0,dim=c(2*length(workers),7))
for(i in 1:length(workers)) {
  for(j in 1:2) {
    d.mat_b_final[i*j, ] <- 
  }
...
p1.111<-p1.11+scale_fill_discrete(name = "", labels = c ("Original", "Copy", "Stranger")) +
  xlab ("") + ylab ("") +
  theme(legend.text = element_text(size = "25"13face = "plain"),
...
quartz()
figure<-ggarrange(p1.1111,p1.1114,p1.1115, nrow=1,ncol=3,common.legend = TRUE, legend=c(0'top'annotate_figure(figure,top = NULL,left = text_grob("d' Performance", color="black",size=27, rot=90)) 
