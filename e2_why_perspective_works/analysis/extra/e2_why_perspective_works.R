...
dir <- setwd("/Users/julian/Documents/github/juliandefreitas/serial_self/e2_why_perspective_works/data")
...
names(data)[15] <- "perspective_cond"

# method 1, with multinom
data$identity2 <- relevel(data$identity_name, ref = "1_original")
test <- multinom(identity2 ~ perspective_cond, data = data)
summary(test)
z <- summary(test)$coefficients/summary(test)$standard.errors; z
p <- (1 - pnorm(abs(z), 0, 1)) * 2; p

# method 2, with mlogit
data2 <- mlogit.data(data, choice = "identity2", shape = "wide")
test <- mlogit(identity2 ~ 1 | perspective_cond, data = data2, reflevel="1_original")
summary(test)
data.frame(exp(test$coefficients))
exp(confint(test))

##two vs. one

# method 1, with multinom
data$two_or_one <- relevel(data$two_or_one, ref = "one")
test <- multinom(two_or_one ~ perspective_cond, data = data)
summary(test)
z <- summary(test)$coefficients/summary(test)$standard.errors; z
p <- (1 - pnorm(abs(z), 0, 1)) * 2; p

# method 2, with mlogit
data2 <- mlogit.data(data, choice = "two_or_one", shape = "wide")
test <- mlogit(two_or_one ~ 1 | perspective_cond, data = data2, reflevel="one")
summary(test)
data.frame(exp(test$coefficients))
exp(confint(test))
...
persp <- relevel(persp, ref = 1)

mod_original <- glm(identity_b_original ~ persp_num, data = data, family=binomial())
summary(mod_original)

#copy
mod_copy <- glm(identity_b_copy ~ persp_num, data = data, family=binomial())
summary(mod_copy)

#neither
mod_neither <- glm(identity_b_neither ~ persp_num, data = data, family=binomial())
summary(mod_neither)
...
mod_both <- glm(identity_b_both ~ persp_num, data = data, family=binomial())
summary(mod_both)
